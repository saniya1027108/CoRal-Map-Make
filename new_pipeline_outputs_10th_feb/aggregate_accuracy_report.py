...
DEFAULT_RESULTS_DIR = REPO_ROOT / "new_pipeline_outputs_10th_feb" / "results"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "new_pipeline_outputs_10th_feb" / "accuracy_report"
...
def build_summary_df(data: dict):
    """DataFrame: one row per trial. Supports new format (summary.summary with Correct/Partial/Wrong) and old format."""
    import pandas as pd
    ...
    for trial, payload in data.items():
        ...
        if "summary" in s and isinstance(s["summary"], dict):
            ...
            row["avg_correctness"] = sm.get("avg_overall")
            ...
            if isinstance(by_cat, list):
                ...
                for cat in ["exact_match", "numeric_tolerance", "structured_text"]:
                    ...
                    row[f"{cat}_overall"] = (
                        (c.get("Correct", 0) * 1.0 + c.get("Partial", 0) * 0.5) / tot if tot else None
                    )
            else:
                for cat in ["exact_match", "numeric_tolerance", "structured_text"]:
...
def build_wrong_only_df(data: dict):
    """DataFrame: one row per (trial, column) where overall < 1.0, with verdict, GT and Pred."""
    import pandas as pd
    ...
    for trial, payload in data.items():
        ...
        for col_name, col_data in cols.items():
            ...
            if overall is not None and overall < 1.0:
                verdict = col_data.get("verdict")  # Correct / Partial / Wrong from new evaluation
                if verdict is None:
                    verdict = "Partial" if overall > 0.0 else "Wrong"
                rows.append({
                    "Trial": trial,
                    "Column": col_name,
                    "verdict": verdict,
...
def build_all_results_df(data: dict):
    """DataFrame: one row per (trial, column) for ALL results, with verdict (Correct/Partial/Wrong)."""
    import pandas as pd
    ...
    for trial, payload in data.items():
        ...
        for col_name, col_data in cols.items():
            ...
            if verdict is None and overall is not None:
                verdict = "Correct" if overall >= 1.0 else ("Partial" if overall > 0.0 else "Wrong")
            rows.append({
                "Trial": trial,
                "Column": col_name,
                "verdict": verdict,
...
def write_excel(data: dict, summary_df, per_column_df, wrong_df, all_results_df, path: Path):
    """Write Excel with Summary, Per-Column (red for wrong), Wrong-Only (with verdict), and All Results (with verdict)."""
    try:
        import openpyxl
        ...
        all_results_df.to_csv(path.with_name(path.stem + ".all_results.csv"), index=False)
        return
    ...
    # Wrong only (includes verdict: Partial / Wrong)
    ...
    # All Results: every (trial, column) with verdict (Correct / Partial / Wrong)
    ws_all = wb.create_sheet("All Results")
    for r in dataframe_to_rows(all_results_df, index=False, header=True):
        ws_all.append(r)
...
<p class="note">Green = correct (1.0), Red = wrong (&lt; 1.0).</p>
...
def main():
    ...
    all_results_df = build_all_results_df(data)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    excel_path = OUTPUT_DIR / "accuracy_report.xlsx"
    write_excel(data, summary_df, per_column_df, wrong_df, all_results_df, excel_path)

    # Always write CSVs (with verdict) for wrong_only and all_results so you can filter by verdict
    wrong_cols = ["Trial", "Column", "verdict", "overall", "correctness", "completeness", "category", "ground_truth", "predicted", "reason"]
    wrong_df.to_csv(OUTPUT_DIR / "accuracy_report.wrong_only.csv", index=False, columns=[c for c in wrong_cols if c in wrong_df.columns])
    all_cols = ["Trial", "Column", "verdict", "overall", "correctness", "completeness", "category", "ground_truth", "predicted", "reason"]
    all_results_df.to_csv(OUTPUT_DIR / "accuracy_report.all_results.csv", index=False, columns=[c for c in all_cols if c in all_results_df.columns])
    ...
    print("  - accuracy_report.xlsx (Summary, Per-Column, Wrong Only, All Results)")
    print("  - accuracy_report.wrong_only.csv (verdict column: Partial/Wrong)")
    print("  - accuracy_report.all_results.csv (verdict: Correct/Partial/Wrong, all rows)")
